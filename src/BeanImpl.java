...
public class BeanImpl implements Bean {
 ...
 //private int xCoor; unused. Fixed advskillTest as well because choose() used xpos and not xCoor
	...
	BeanImpl(int slotCount, boolean isLuck, Random rand) {
		...
		random = rand;
		//double for accuracy
		double skillAverage = 0.5 * ((double) (slotCount - 1));
		double skillStdev = (double) Math.sqrt(slotCount * 0.5 * (1 - 0.5));
		skillLevel = (int) Math.round(random.nextGaussian() * skillStdev + skillAverage);
